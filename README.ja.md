...
| Bluetooth経路 | 無効 | 現在使用中のHome Assistant BluetoothスキャナーまたはESPHome Proxyと、source／RSSIの診断情報 |
| Bluetooth経路の選択 | 無効 | 次回の手動再接続で使用する経路。再起動後は自動選択に戻ります |
| 選択した経路で再接続 | 無効 | 選択したBluetooth経路を使って手動で再接続 |
